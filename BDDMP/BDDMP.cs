...
namespace BDDMP
{
	...
	{
		//public static BDDMPSynchronizer singleton;
        //FX Sync
        ...
        {
            if (dmpcli.dmpGame.vesselWorker.LenientVesselUpdatedInFuture(vesselID)) {
                ScreenMessages.PostScreenMessage("BDArmory-DMP: Cannot damage vessel from the past!", 3f, ScreenMessageStyle.UPPER_LEFT);
            }

            return !dmpcli.dmpGame.vesselWorker.LenientVesselUpdatedInFuture (vesselID);
        }

        private bool CanControl()
        {
            Guid vesselID = FlightGlobals.ActiveVessel.id;
            if (dmpcli.dmpGame.vesselWorker.LenientVesselUpdatedInFuture(vesselID) || dmpcli.dmpGame.vesselWorker.isSpectating)
            {
            ...
            return !dmpcli.dmpGame.vesselWorker.LenientVesselUpdatedInFuture(vesselID) && !dmpcli.dmpGame.vesselWorker.isSpectating;
        }
        #endregion
	}
